...
class EntityComponentSystemManager:
    def __init__(self):
        ...
        signaler.instance.register("breakpoint", self.breakpoint)
    ...
    def init_entities(self):
        for entity_name in config.entities:
            ...
            entity.components = sorted(components, key=lambda c: c.__class__.__name__)
    ...
    def has_system(self, system):
        return system in self.systemdb[system.componenttypes]

    def remove_system(self, system: System):
        self.systemdb[system.componenttypes].remove(system)

    def add_entity(self, entity):
        combs, comps = [], list(
            sorted([type(c) for c in entity.components], key=lambda x: x.__name__)
        )
    ...
    def process(self, elapsed):
        for componenttypes in self.systemdb:
            for system in self.systemdb[componenttypes]:
                ...
                system.process(
                    signaler=self.signaler, components=comps, elapsed=elapsed
                )
